...
LPDIRECTDRAWSURFACE4 g_pDdsZBuffer = NULL;
...
LPDIRECT3DTEXTURE2 Get_Texture(char *szFilename)
{
	...
	HBITMAP oldbmp = (HBITMAP)SelectObject(memdc, hbmBitmap);
	GetDIBColorTable(memdc, 0, 256, (RGBQUAD*)RgbPal);
	...
	ZeroMemory( &ddsd, sizeof(DDSURFACEDESC2) );
    ddsd.dwSize = sizeof(DDSURFACEDESC2);
    TexSurface->Lock( NULL, &ddsd, DDLOCK_WRITEONLY, NULL );

	unsigned char *pSrc = (unsigned char *)bm.bmBits;
	unsigned char *pDest = (unsigned char*)ddsd.lpSurface;

	for ( int h = 0; h < bm.bmHeight; h++ )
	{
		for ( int w = 0; w < bm.bmWidth; w++)
		{
			//вычисляем индексы в массиве
			int IndexSrc = h * bm.bmWidth + w;
			int IndexDst = (h * bm.bmHeight + w) * 4;

			//получаем номер цвета в палитре
			int Color = pSrc[IndexSrc];
			
			//извлекаем цвета RGB из палитры
			pDest[IndexDst + 0] = RgbPal[Color].rgbBlue;
			pDest[IndexDst + 1] = RgbPal[Color].rgbGreen;
			pDest[IndexDst + 2] = RgbPal[Color].rgbRed;
			pDest[IndexDst + 3] = 0;
		}
	}
	
	TexSurface->Unlock(NULL);
...
HRESULT WINAPI Enum_ZBuffer_Callback( DDPIXELFORMAT* pddpf,
                                           VOID* pddpfDesired )
{
...
HRESULT Initialize_3DEnvironment()
{
	...
	g_pD3D->EnumZBufferFormats( *pDeviceGUID, 
                                Enum_ZBuffer_Callback, (VOID*)&ddpfZBuffer );
	/*
	//можно и так
...
void Init_Scene()
{
	...
	//если в функции Render_Scene() закоментировать
	//очистку Z буфера D3DCLEAR_ZBUFFER, поставить ниже
	//cull mode D3DCULL_NONE, и в функции Initialize_3DEnvironment()
	//закоментировать создание Z буфера - то можно убедиться
	//в работе и необходимости Z буфера - треугольники куба
	//будут выводиться на экран в хаотичном порядке
	//без учета глубины пикселей
	...
	g_pCubeTexture = Get_Texture("texture8.bmp");
...
HRESULT Render_Scene()
{
	...
	HRESULT hr = g_pViewport->Clear2( 1UL, (D3DRECT*)&g_RcViewportRect, D3DCLEAR_TARGET | D3DCLEAR_ZBUFFER,
